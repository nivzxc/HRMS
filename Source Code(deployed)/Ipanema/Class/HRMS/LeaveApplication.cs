...
namespace HRMS
{
 public class LeaveApplication : IDisposable
 {
  ...
  public bool InsertAdmin()
  {
   ...
   try
   {
    cmd.CommandText = "SELECT pvalue FROM Speedo.Keys WHERE pkey='leavcode'";
    intSeed = int.Parse(cmd.ExecuteScalar().ToString());
    _strLeaveCode = ("000000000" + intSeed.ToString()).Substring(intSeed.ToString().Length);
