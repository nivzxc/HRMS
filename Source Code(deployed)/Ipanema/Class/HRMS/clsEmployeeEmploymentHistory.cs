...
namespace HRMS
{
 public class clsEmployeeEmploymentHistory : IDisposable
 {
  ...
  public static DataTable DSGFormEmployeeDetailsList(string pUsername)
  {
   DataTable tblReturn = new DataTable();
   tblReturn.Columns.Add("EmhsCode");
   tblReturn.Columns.Add("InclusiveDates");
   tblReturn.Columns.Add("Company");
   tblReturn.Columns.Add("Position");
   tblReturn.Columns.Add("AppointmentStatus");

   using (SqlConnection cn = new SqlConnection(HRMSCore.HrmsConnectionString))
   {
    SqlCommand cmd = cn.CreateCommand();
    cmd.CommandText = "SELECT * FROM HR.EmployeeEmploymentHistory WHERE username='" + pUsername + "' ORDER BY emhscode";
    cn.Open();
    SqlDataReader dr = cmd.ExecuteReader();
    while (dr.Read())
    {
     DataRow drwN = tblReturn.NewRow();
     drwN["EmhsCode"] = dr["emhscode"].ToString();
     drwN["InclusiveDates"] = dr["incldate"].ToString();
     drwN["Company"] = dr["compname"].ToString();
     drwN["Position"] = dr["position"].ToString();
     drwN["AppointmentStatus"] = clsEmploymentStatus.GetEmploymentStatusName(dr["esttcode"].ToString());
