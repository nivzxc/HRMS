  ...
  public void Fill()
  {
   using (SqlConnection cn = new SqlConnection(HRMSCore.HrmsConnectionString))
   {
    ...
    if (dr.Read())
    {
     ...
     _dtePeriodFrom = clsDateTime.ChangeTimeToStart(clsValidator.CheckDate(dr["tspfrom"].ToString()));
     _dtePeriodTo = clsDateTime.ChangeTimeToEnd(clsValidator.CheckDate(dr["tspto"].ToString()));
